...
\begin{abstract}
This note summarizes the current HAOS-IIP operator program at the level that is now supported by explicit numerics. A single interaction kernel defines a weighted discrete substrate, but different operator lifts act on different field types. The node Laplacian $L_0$ produces the scalar or geometry branch. The edge or Hodge operator $L_1$ produces the first non-scalar branch. The key numerical result is that the low periodic $L_1$ spectrum is not reducible to scalar-gradient leakage: on a cubic $3$-torus with background plaquette holonomy, a low mostly divergence-free branch persists across the tested lattice sizes and responds to flux. The same branch is still best interpreted as flux-deformed harmonic or topological edge structure rather than a clean propagating Maxwell-like band. No claim of emergent photons, full gauge theory, or particle matching is made here.
\end{abstract}
...
\end{align}
Only the first two levels are numerically active in the present repository. The node sector is already supported by explicit low-mode studies on concrete $3$D substrates. The edge sector now has a working periodic and twisted implementation together with a wider flux scan. The main question addressed in this paper is therefore narrow: does the current edge operator contain a genuine low non-scalar branch, or is the apparent vector content still inherited from scalar gradients?
...
\lambda_3 = 2.422536.
\end{equation}
These three modes form a near-degenerate smooth family aligned with the coordinate directions of the box. Their inverse participation ratios remain low, and the visual mode shapes match the first scalar box modes. This is the current evidence that the node branch reproduces geometry-like long-wavelength behavior.
...
The present implementation also has an important limitation. The reported low-sector fraction is computed using projection onto $\ker d_{0,A}^*$, that is, the divergence-free subspace. On a torus this subspace contains both harmonic and coexact components. Accordingly, the current numerics do not yet isolate a purely coexact propagating band.

At the same time, the present branch is not yet a clean Maxwell-like propagating band. In the trivial-flux case, the lowest modes are harmonic torus cycles. Under flux, those zero modes lift into small positive eigenvalues and develop circulation structure, but the low branch is still best described as flux-deformed harmonic or topological edge structure rather than a separated transverse continuum family.
...
\begin{quote}
The repository now contains operator-level evidence for a low vector-like branch in $L_1$. That branch is mostly divergence free and flux responsive on topologically nontrivial substrates, but it is not yet a clean emergent photon sector or a dynamical gauge field.
\end{quote}
...
First, the periodic low branch must be separated more cleanly into harmonic, coexact, and eventually propagating components. The current diagnostics establish non-scalar structure, but they do not yet isolate a smooth transverse band.

Second, the next technical step is to refine the Hodge-sector decomposition by separating harmonic and coexact subspaces explicitly rather than using only the divergence-free projector onto $\ker d_{0,A}^*$. Only after this separation is numerically stable should the program broaden toward defect-localized circulation modes or a dynamical plaquette action for the link variables.

Third, the periodic torus is not sufficient by itself. Larger lattice sizes, punctures, and defect backgrounds are needed to test whether the low divergence-free family remains delocalized, becomes localized around topological structure, or reorganizes into a more continuum-like circulation sector.
...
\begin{enumerate}
\item more explicit harmonic versus coexact separation,
\item larger periodic flux scans,
\item puncture and defect backgrounds,
\item continuum scaling tests for the low edge branch,
