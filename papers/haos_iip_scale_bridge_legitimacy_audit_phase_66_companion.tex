...
\item initial convergence diagnostic extract recorded in Section 14;
\item next target: same-surrogate coarse-graining recovery before any stronger scale-bridge statement.
\end{itemize}
...
\section{v1.2 Initial Convergence Diagnostic Extract}
...
\subsection{Metric-Surrogate Refinement}

The first table uses the Phase XVIII distance-surrogate shell slope, \texttt{mean\_arrival\_vs\_depth\_slope}, reconstructed on the \texttt{bias\_onset}, ensemble-7 slice. The matched control is \texttt{periodic\_diagonal\_augmented\_control}.

\begin{center}
\scriptsize
\begin{tabular}{r r r r r r p{0.18\linewidth}}
\toprule
...
\subsection{Projection-Recovery Compression}

The second table records Phase X low-mode spectral projection under cutoff \(\lambda \le 7.5\). It supports compression bookkeeping, not branch-specific universality.

\begin{center}
\scriptsize
\begin{tabular}{l r r r r r p{0.18\linewidth}}
\toprule
...
\subsection{Matched-Control Separation}

\begin{center}
\small
\begin{tabular}{p{0.18\linewidth}p{0.26\linewidth}p{0.20\linewidth}p{0.20\linewidth}}
\toprule
Control & What was matched & Branch result & Control result \\
\midrule
periodic diagonal augmented control & disturbance, seeds, ensemble size, arrival/depth observable, refinement levels & max slope drift \(0.004762\); causal-depth drift \(0.000000\) & max slope drift \(0.584615\); causal-depth drift \(0.714286\) \\
\bottomrule
\end{tabular}
\end{center}
