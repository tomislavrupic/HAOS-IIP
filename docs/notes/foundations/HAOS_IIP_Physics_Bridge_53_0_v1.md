...
> HAOS-IIP now has an external physics-bridge readout showing which scalar-carrier quantities behave like metric, shell-native power-law / inverse-square response, weak localized-excitation, and transport proxies under the current frozen thresholds, while explicitly preserving the raw inverse-square residual, raw power-law scaling, strong localized-bump, and seed-universal disorder boundaries as open or watched.
