...
| --- | --- | --- | --- | --- | --- | --- |
| Spectral address | Recoverable spectral / cochain identity coordinate | A stable, recoverable resonance pattern or eigenmode-like signature inside the frozen branch-local cochain-Laplacian hierarchy; not static data, but a signature reconstructable from interaction invariants and recoverable cochain structures | Laplacian eigenmode or subspace identity, spectral drift, projection retention, cochain-invariant reconstruction, branch/control separation | A guaranteed physical particle, field, hidden essence, universal frequency, or static memory cell | Laplacian eigenmodes, normal modes, spectral graph theory, Hodge/cochain spectra, spectral clusters, coherent modes | The address cannot be reidentified, fails reconstruction, drifts into controls, aliases, broadens, or is indistinguishable from arbitrary labeling |
| Harmonic address | Legacy/internal shorthand for spectral-address framing | Historical phrase for the broader intuition that stable resonance-like identities can be addressed; public technical work should translate it into spectral address or stable spectral / operator identity class first | Same gates as spectral address when used technically: identity matching, recovery under perturbation, and control separation | A universal resonance, hidden essence, or proof of physical ontology | Eigenmodes, normal modes, spectral clusters, coherent modes | The phrase is used as explanation without an operational spectral/cochain construction |
