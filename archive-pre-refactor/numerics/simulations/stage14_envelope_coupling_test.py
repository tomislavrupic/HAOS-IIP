...
from stage14_branch_terms import envelope_coupling_profiles
...
RUNSHEET_PATH = REPO_ROOT / 'numerics' / 'simulations' / 'stage14_envelope_coupling_runs.json'
SIGMAS = (2.0, 4.0)
ALPHA = 0.6
ANALYSIS_STRIDE = 4
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run the Stage 14C Branch C envelope-mediated coupling pilot.')
...
def simulate_run(rep: dict[str, Any], condition: dict[str, Any], resolution: int, defaults: dict[str, Any], base: dict[str, Any], coupling_sigma: float, overlap_threshold: float, work_plot_dir: Path) -> tuple[dict[str, Any], list[Path]]:
    ...
    branch_eps = float(condition['branch_eps'])
    ...
    for step_idx in range(steps):
        coupling_profiles, _envs = envelope_coupling_profiles(data.midpoints, packet_qs, resolution, coupling_sigma)
        next_qs: list[np.ndarray] = []
        next_vs: list[np.ndarray] = []
        for q, v, coupling_profile in zip(packet_qs, packet_vs, coupling_profiles):
            coupling_term = branch_eps * coupling_profile * q
            acc = np.asarray(projector(-(operator @ q) + coupling_term), dtype=float)
            v_half = v + 0.5 * dt * acc
            q_new = np.asarray(projector(q + dt * v_half), dtype=float)
            acc_new = np.asarray(projector(-(operator @ q_new) + branch_eps * coupling_profile * q_new), dtype=float)
    ...
    fig.suptitle(f"Stage 14C: {row['run_id']}")
...
def create_comparison_plot(path: Path, base_rows: list[dict[str, Any]]) -> None:
    ...
    cond_labels = {
        'null_control': 'null',
        'very_weak_coupling': 'very weak',
        'weak_coupling': 'weak',
    }
    cond_order = ['null_control', 'very_weak_coupling', 'weak_coupling']
    ...
    fig.suptitle('Stage 14C null vs coupling comparison')
...
def create_regime_map(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    conds = ['null_control', 'very_weak_coupling', 'weak_coupling']
    coarse_map = {
    ...
    ax.set_title('Stage 14C coarse morphology regime map')
    plt.colorbar(im, ax=ax, fraction=0.045, pad=0.04)
    fig.savefig(path, dpi=180, bbox_inches='tight')
    plt.close(fig)
...
def create_resolution_plot(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    fig.suptitle('Stage 14C refinement follow-up')
    fig.savefig(path, dpi=180, bbox_inches='tight')
    plt.close(fig)
...
def write_note(path: Path, json_rel: str, csv_rel: str, base_rows: list[dict[str, Any]], promoted: dict[str, Any] | None, refined_rows: list[dict[str, Any]], robustness_counts: dict[str, int]) -> None:
    lines = [
        '# Stage 14C Envelope-Mediated Coupling v1',
        '',
        ...
        '',
        'This pilot tests only Stage 14C Branch C: weak envelope-mediated coupling against frozen Stage 13 null controls.',
        '',
        'Base comparisons:',
        '',
    ]
...
def main() -> None:
    ...
    for rep in representatives:
        ...
        for condition in conditions:
            payload, run_plots = simulate_run(
                ...
                defaults=defaults,
                base=base,
                coupling_sigma=float(runsheet['coupling_sigma']),
    ...
    result = {
        ...
        'conclusion': 'Stage 14C compares null control against very weak and weak envelope-mediated coupling while preserving Stage 13 comparability.',
    }
    ...
    append_log(
        title=f"Stage 14C Envelope Coupling ({json_path.stem})",
