...
NOTE_PATH = REPO_ROOT / 'experiments' / 'gauge_diagnostics' / 'Loop_Response_Extended_Test_v1.md'
...
def edge_value(data, field, axis, i, j, k, sign):
    idx = data.edge_map[(axis, i % data.n_side, j % data.n_side, k % data.n_side)]
...
def main() -> None:
    ...
    _, raw_vecs = low_eigensystem(transverse_operator, 12, eig_tol)
    ...
    plot_per = REPO_ROOT / 'plots' / 'loop_response_extended_perimeter.png'
    fig, ax = plt.subplots(figsize=(6.2, 4.8))
    ax.loglog(perimeter, response, marker='o')
    ax.loglog(perimeter, perimeter_fit['prefactor'] * np.asarray(perimeter) ** perimeter_fit['power'], '--', label=f"perimeter p={perimeter_fit['power']:.3f}, R$^2$={perimeter_fit['r2']:.4f}")
    ax.set_xlabel('loop perimeter')
    ax.set_ylabel('mean |loop response|')
    ax.set_title('Stage 8 loop response refinement vs perimeter')
    ax.grid(alpha=0.25, which='both')
    ax.legend(fontsize=8)
    fig.savefig(plot_per, dpi=180, bbox_inches='tight')
    plt.close(fig)

    plot_area = REPO_ROOT / 'plots' / 'loop_response_extended_area.png'
    fig, ax = plt.subplots(figsize=(6.2, 4.8))
    ax.loglog(area, response, marker='o')
    ax.loglog(area, area_fit['prefactor'] * np.asarray(area) ** area_fit['power'], '--', label=f"area p={area_fit['power']:.3f}, R$^2$={area_fit['r2']:.4f}")
    ax.set_xlabel('loop area')
    ax.set_ylabel('mean |loop response|')
    ax.set_title('Stage 8 loop response refinement vs area')
    ax.grid(alpha=0.25, which='both')
    ax.legend(fontsize=8)
    ...
    result = {
        'experiment': 'stage8_loop_response_extended',
        'config': {
            'n_side': n_side,
            'epsilon': epsilon,
    ...
    stamped_json, stamped_plots, _ = save_result_payload('stage8_loop_response_extended', result, [plot_per, plot_area])
    append_log(
    ...
    NOTE_PATH.write_text(
        f"# Loop Response Extended Test v1\n\n"
