...
def build_note(result: dict[str, Any]) -> str:
    ...
    for record in result['flux_cases']:
        rows.append(
            f"| {record['cycle_phase_x']:.6f} | {record['square_relative_residual']:.3e} | {record['first_positive_mode']:.6f} | {record['last_tracked_mode']:.6f} | {record['omega1_fraction_mean']:.3f} |"
        )
    table = '\n'.join(rows)
    return f"""# DK 3D Flux Response Test
...
- x-cycle phases: `{result['config']['cycle_phases']}`
...
def run_dk_3d_flux_response_test(config: dict[str, Any] | None = None) -> tuple[dict[str, Any], Path, Path]:
    cfg = load_config(config)
    stage_cfg = cfg['DK_stage7']
    epsilon = float(cfg.get('epsilon', 0.2))
    n_side = int(stage_cfg.get('flux_n', 4))
    ...
    for phase in cycle_phases:
        complex_data = build_dk3d_complex(n_side=n_side, epsilon=epsilon, cycle_phase_x=phase)
        ...
        records.append(
            {
                ...
                'first_positive_mode': float(positives[0]),
                'last_tracked_mode': float(positives[-1]),
    ...
    flow_path = PLOTS / f'{timestamp}_DK_3D_flux_spectral_flow.png'
    residual_path = PLOTS / f'{timestamp}_DK_3D_flux_square_residual.png'
    ...
    result_path = RESULTS / f'{timestamp}_DK_3D_flux_response_test.json'
    result = {
        'experiment': 'DK_3D_flux_response_test',
        'timestamp': timestamp,
        'config': {
            'epsilon': epsilon,
            'n_side': n_side,
    ...
    note_path = REPO_ROOT / 'experiments' / 'spinor_sector' / 'DK_3D_Flux_Response_Test_v1.md'
    note_path.write_text(build_note(result), encoding='utf-8')
    append_log(
        title='DK 3D Flux Response Test',
