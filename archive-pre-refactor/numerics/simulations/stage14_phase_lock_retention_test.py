...
from stage14_branch_terms import phase_lock_retention_profile
...
RUNSHEET_PATH = REPO_ROOT / 'numerics' / 'simulations' / 'stage14_phase_lock_runs.json'
SIGMAS = (2.0, 4.0)
ALPHA = 0.6
ANALYSIS_STRIDE = 4
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run the Stage 14B Branch B phase-lock retention pilot.')
...
def simulate_run(rep: dict[str, Any], condition: dict[str, Any], resolution: int, defaults: dict[str, Any], base: dict[str, Any], phase_sigma: float, overlap_threshold: float, work_plot_dir: Path) -> tuple[dict[str, Any], list[Path]]:
    ...
    phase_memory = np.zeros_like(total_q.real)
    for step_idx in range(steps):
        retention_profile, _ = phase_lock_retention_profile(data.midpoints, packet_qs, resolution, phase_sigma)
        phase_memory = 0.9 * phase_memory + 0.1 * retention_profile
        next_qs: list[np.ndarray] = []
        next_vs: list[np.ndarray] = []
        for q, v in zip(packet_qs, packet_vs):
            retention_term = branch_eps * phase_memory * q
            acc = np.asarray(projector(-(operator @ q) + retention_term), dtype=float)
            v_half = v + 0.5 * dt * acc
            q_new = np.asarray(projector(q + dt * v_half), dtype=float)
            acc_new = np.asarray(projector(-(operator @ q_new) + branch_eps * phase_memory * q_new), dtype=float)
    ...
    fig.suptitle(f"Stage 14B: {row['run_id']}")
...
def create_comparison_plot(path: Path, base_rows: list[dict[str, Any]]) -> None:
    ...
    cond_labels = {
        'null_control': 'null',
        'very_weak_retention': 'very weak',
        'weak_retention': 'weak',
    }
    cond_order = ['null_control', 'very_weak_retention', 'weak_retention']
    ...
    fig.suptitle('Stage 14B null vs retention comparison')
...
def create_regime_map(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    conds = ['null_control', 'very_weak_retention', 'weak_retention']
    coarse_map = {
    ...
    ax.set_title('Stage 14B coarse morphology regime map')
    plt.colorbar(im, ax=ax, fraction=0.045, pad=0.04)
    fig.savefig(path, dpi=180, bbox_inches='tight')
    plt.close(fig)
...
def create_resolution_plot(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    fig.suptitle('Stage 14B refinement follow-up')
    fig.savefig(path, dpi=180, bbox_inches='tight')
    plt.close(fig)
...
def write_note(path: Path, json_rel: str, csv_rel: str, base_rows: list[dict[str, Any]], promoted: dict[str, Any] | None, refined_rows: list[dict[str, Any]], robustness_counts: dict[str, int]) -> None:
    lines = [
        '# Stage 14B Phase-Lock Retention v1',
        '',
        ...
        '',
        'This pilot tests only Stage 14B Branch B: weak phase-lock retention against frozen Stage 13 null controls.',
        '',
        'Base comparisons:',
        '',
    ]
...
def main() -> None:
    ...
    for rep in representatives:
        ...
        for condition in conditions:
            payload, run_plots = simulate_run(
                ...
                defaults=defaults,
                base=base,
                phase_sigma=float(runsheet['phase_sigma']),
    ...
    result = {
        ...
        'conclusion': 'Stage 14B compares null control against very weak and weak phase-lock retention while preserving Stage 13 comparability.',
    }
    ...
    append_log(
        title=f"Stage 14B Phase-Lock Retention ({json_path.stem})",
