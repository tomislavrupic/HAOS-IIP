...
NOTE_PATH = WAVE_NOTES / 'DK_3D_Packet_Dynamics_v1.md'


def analyze_variant(label: str, active_grades: list[int], complex_data, cfg: dict) -> dict:
    positions = pack_positions(complex_data.points, complex_data.edge_midpoints, complex_data.face_centers, complex_data.volume_centers)
    psi0 = build_initial_packet(
        positions=positions,
        block_sizes=complex_data.block_sizes,
        center=np.asarray(cfg['packet_center_3d'], dtype=float),
        sigma=float(cfg['sigma_3d']),
    ...
    steps = max(1, int(np.ceil(float(cfg['t_final_3d']) / dt)))
    ...
    return {
        ...
        'summary': {
            ...
            'low_mode_participation_proxy': float(np.mean(grade_hist[:, :2].sum(axis=1))),
        },
    }
...
def main() -> None:
    ...
    n_side = int(cfg['n_3d'])
    complex_data = build_dk3d_complex(n_side=n_side, epsilon=float(cfg['epsilon']))
    pairing = spectral_pairing_summary_3d(complex_data.dirac_kahler, modes=int(cfg['low_modes_3d']), tol=1.0e-9)

    variants = [
        analyze_variant('omega1_packet', [1], complex_data, cfg),
        analyze_variant('mixed_packet', [0, 1, 2, 3], complex_data, cfg),
    ]

    plot_position = REPO_ROOT / 'plots' / 'DK_3D_packet_position.png'
    ...
    ax.set_title(f'3D DK packet trajectories (n={n_side})')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    fig.savefig(plot_position, dpi=180, bbox_inches='tight')
    plt.close(fig)

    plot_width = REPO_ROOT / 'plots' / 'DK_3D_packet_width.png'
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    for case in variants:
        ax.plot(case['times'], case['widths'], label=case['label'])
    ax.set_xlabel('time')
    ax.set_ylabel('packet width')
    ax.set_title(f'3D DK packet width evolution (n={n_side})')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    fig.savefig(plot_width, dpi=180, bbox_inches='tight')
    plt.close(fig)

    plot_norm = REPO_ROOT / 'plots' / 'DK_3D_packet_norm.png'
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    for case in variants:
    ...
    ax.set_title(f'3D DK norm preservation (n={n_side})')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    fig.savefig(plot_norm, dpi=180, bbox_inches='tight')
    plt.close(fig)

    plot_grades = REPO_ROOT / 'plots' / 'DK_3D_packet_grade_weights.png'
    ...
    for ax, case in zip(axes, variants):
        ...
        ax.stackplot(case['times'], grade_hist.T, labels=[r'$\Omega^0$', r'$\Omega^1$', r'$\Omega^2$', r'$\Omega^3$'], alpha=0.85)
    ...
    plot_anisotropy = REPO_ROOT / 'plots' / 'DK_3D_packet_anisotropy.png'
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    for case in variants:
        ax.plot(case['times'], case['anisotropies'], label=case['label'])
    ax.set_xlabel('time')
    ax.set_ylabel('anisotropy ratio')
    ax.set_title(f'3D DK packet anisotropy (n={n_side})')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    fig.savefig(plot_anisotropy, dpi=180, bbox_inches='tight')
    plt.close(fig)
    ...
    result = {
        'experiment': 'stage9b_DK_3D_packet_dynamics',
        'config': {
            'n_side': n_side,
            'epsilon': float(cfg['epsilon']),
            'sigma_3d': float(cfg['sigma_3d']),
            'kick_cycles': float(cfg['kick_cycles']),
            't_final_3d': float(cfg['t_final_3d']),
        },
    ...
    stamped_json, stamped_plots, _ = save_result_payload('DK_3D_packet_dynamics', result, [plot_position, plot_width, plot_norm, plot_grades, plot_anisotropy])
    append_log(
        'Stage 9B DK 3D Packet Propagation',
        f"n={n_side}, epsilon={cfg['epsilon']}, sigma={cfg['sigma_3d']}, t_final={cfg['t_final_3d']}",
        stamped_json,
        stamped_plots,
        observation,
        conclusion,
    ...
    NOTE_PATH.write_text(
        f"# DK 3D Packet Dynamics v1\n\n"
        f"Timestamped result: `{stamped_json.relative_to(REPO_ROOT)}`\n\n"
        f"Config: n={n_side}, epsilon={cfg['epsilon']}, sigma={cfg['sigma_3d']}, kick_cycles={cfg['kick_cycles']}, "
        f"t_final={cfg['t_final_3d']}.\n\n"
