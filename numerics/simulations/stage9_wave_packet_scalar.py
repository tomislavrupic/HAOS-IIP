...
NOTE_PATH = WAVE_NOTES / 'Scalar_Wave_Packet_Test_v1.md'


def analyze_case(n_side: int, cfg: dict) -> dict:
    data = build_periodic_complex(n_side=n_side, epsilon=float(cfg['epsilon']))
    q0, v0 = build_scalar_initial_state(
        points=data.points,
        center=np.asarray(cfg['packet_center'], dtype=float),
        sigma=float(cfg['sigma']),
        kick_axis=int(cfg['kick_axis']),
        kick_strength=float(cfg['kick_strength_scalar']),
    )
    dt = suggested_dt(data.L0, float(cfg['dt_safety']))
    steps = max(1, int(np.ceil(float(cfg['t_final']) / dt)))
    run = simulate_second_order(data.L0, q0, v0, data.points, dt, steps)
...
def main() -> None:
    ...
    plot_position = REPO_ROOT / 'plots' / 'scalar_packet_position.png'
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    for case in cases:
        times = np.asarray(case['times'], dtype=float)
        centers = np.asarray(case['centers'], dtype=float)
        ax.plot(times, centers[:, 0], label=f"n={case['n_side']}")
    ax.set_xlabel('time')
    ax.set_ylabel('packet center x')
    ax.set_title('Scalar wave-packet center trajectory')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    fig.savefig(plot_position, dpi=180, bbox_inches='tight')
    plt.close(fig)

    plot_width = REPO_ROOT / 'plots' / 'scalar_packet_width.png'
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    for case in cases:
        ax.plot(case['times'], case['widths'], label=f"n={case['n_side']}")
    ax.set_xlabel('time')
    ax.set_ylabel('packet width')
    ax.set_title('Scalar wave-packet width evolution')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    fig.savefig(plot_width, dpi=180, bbox_inches='tight')
    plt.close(fig)

    plot_energy = REPO_ROOT / 'plots' / 'scalar_packet_energy.png'
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    for case in cases:
    ...
    ax.set_title('Scalar wave-packet energy conservation')
    ax.grid(alpha=0.25)
    ax.legend(fontsize=8)
    ...
    result = {
        'experiment': 'stage9_scalar_packet_dynamics',
        'config': {
            'sizes': sizes,
            'epsilon': float(cfg['epsilon']),
            'sigma': float(cfg['sigma']),
            'kick_axis': int(cfg['kick_axis']),
            'kick_strength_scalar': float(cfg['kick_strength_scalar']),
    ...
    stamped_json, stamped_plots, timestamp = save_result_payload('stage9_scalar_packet_dynamics', result, [plot_position, plot_width, plot_energy])
    append_log(
    ...
    NOTE_PATH.write_text(
        ...
        f"kick_strength={cfg['kick_strength_scalar']}, t_final={cfg['t_final']}.\n\n"
