...
RUNSHEET_PATH = REPO_ROOT / 'numerics' / 'simulations' / 'stage19d_mixed_scalar_vector_runs.json'
...
WORK_PLOT_DIR = Path('/tmp') / 'haos_iip_stage19d_mixed_scalar_vector'
...
CSV_FIELDS = [
    ...
    'scalar_signal_strength',
    'vector_signal_strength',
    'scalar_support_fraction',
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run a Stage 19D mixed scalar-vector substrate pilot.')
...
def branch_kind_from_runsheet(runsheet: dict[str, Any]) -> str:
    ...
    if coupling_class == 'mixed_scalar_vector_substrate_reinforcement':
        return 'mixed_scalar_vector'
    return str(runsheet.get('kernel_law', {}).get('substrate_law', 'mixed_scalar_vector'))


def default_note_name(branch_kind: str) -> str:
    return 'Stage_19D_Mixed_Scalar_Vector_Substrate_Reinforcement_v1.md'


def default_experiment_slug(branch_kind: str) -> str:
    return 'stage19d_mixed_scalar_vector'


def normalize_stage19d_runsheet(runsheet: dict[str, Any]) -> dict[str, Any]:
    ...
    for run in runsheet['runs']:
        ...
        normalized_runs.append({
            ...
            'substrate_law': 'mixed_scalar_vector',
            'condition_id': condition,
            'eps_s': float(run['eps_s']),
            'eps_v': float(run['eps_v']),
...
def condition_label(condition_id: str) -> str:
    labels = {
        'null_control': 'null',
        'null': 'null',
        'balanced_low': '0.01/0.01',
        'balanced_moderate': '0.02/0.02',
    }
...
def simulate_run(
    ...
    branch_kind = str(runsheet.get('branch_kind', 'mixed_scalar_vector'))
    ...
    tau_memory = float(common_fields.get('memory_tau', estimate_crossing_time(base, dt)))
    tau_memory = max(tau_memory, 2.0 * dt)
    decay = max(0.0, 1.0 - dt / tau_memory)
    leakage_fn = make_leakage_fn(projector)

    scalar_field = np.zeros_like(baseline_edge_weights)
    vector_field = np.zeros_like(baseline_edge_weights)
    ...
    scalar_norm_series: list[float] = []
    vector_norm_series: list[float] = []
    mixed_norm_series: list[float] = []
    energy_dispersion_series: list[float] = []
    baseline_energy = 0.0
    directional_signal_series: list[float] = []
    directional_support_series: list[float] = []
    transport_anisotropy_series: list[float] = []
    scalar_signal_series: list[float] = []
    scalar_support_series: list[float] = []
    ...
    def record(t: float, operator: sp.csr_matrix) -> None:
        ...
        current_energy = state_energy(operator, total_q, total_v)
        ...
        scalar_norm_series.append(float(np.linalg.norm(scalar_field) / math.sqrt(max(len(scalar_field), 1))))
        vector_norm_series.append(float(np.linalg.norm(vector_field) / math.sqrt(max(len(vector_field), 1))))
        mixed_norm_series.append(float(np.linalg.norm(eps_s * scalar_field + eps_v * vector_field) / math.sqrt(max(len(vector_field), 1))))
        energy_dispersion_series.append(float(current_energy))
    ...
    for step_idx in range(steps):
        total_q = np.sum(packet_qs, axis=0)
        total_v = np.sum(packet_vs, axis=0)
        scalar_signal, scalar_support = scalar_density_signal(
            data.midpoints,
            total_q,
            resolution,
            float(common_fields.get('scalar_smoothing_sigma', 2.0)),
        )
        signal, support_fraction, transport_anisotropy = directional_flux_signal(
            ...
            data.edge_axes,
            resolution,
            float(common_fields.get('vector_smoothing_sigma', 2.0)),
        )
        scalar_signal_series.append(float(np.mean(np.abs(scalar_signal))))
        scalar_support_series.append(float(scalar_support))
        ...
        if eps_s > 0.0 or eps_v > 0.0:
            scalar_centered = scalar_signal - float(np.mean(scalar_signal))
            vector_centered = signal - float(np.mean(signal))
            scalar_field = decay * scalar_field + scalar_centered
            vector_field = decay * vector_field + vector_centered
            scalar_cap = kappa_max / max(eps_s, 1.0e-12) if eps_s > 0.0 else 0.0
            vector_cap = kappa_max / max(eps_v, 1.0e-12) if eps_v > 0.0 else 0.0
            if eps_s > 0.0:
                scalar_field = np.clip(scalar_field, -scalar_cap, scalar_cap)
            else:
                scalar_field[:] = 0.0
            if eps_v > 0.0:
                vector_field = np.clip(vector_field, -vector_cap, vector_cap)
            else:
                vector_field[:] = 0.0
            scale = (1.0 + eps_s * scalar_field) * np.exp(eps_v * vector_field)
            scale = np.clip(scale, 1.0 - kappa_max, 1.0 + kappa_max)
    ...
    row = {
        ...
        'scalar_field_norm': float(np.max(scalar_norm_series)) if scalar_norm_series else 0.0,
        'vector_field_norm': float(np.max(vector_norm_series)) if vector_norm_series else 0.0,
        'mixed_field_norm': float(np.max(mixed_norm_series)) if mixed_norm_series else 0.0,
        ...
        'scalar_signal_strength': float(np.mean(scalar_signal_series)) if scalar_signal_series else 0.0,
        'vector_signal_strength': float(np.mean(directional_signal_series)) if directional_signal_series else 0.0,
        'scalar_support_fraction': float(np.mean(scalar_support_series)) if scalar_support_series else 0.0,
        ...
        'mixed_transport_stabilized': 0,
        'gate_met': 0,
        'promoted_followup': 0,
        'output_label': 'no mixed substrate effect',
    ...
    trace_path = WORK_PLOT_DIR / f"stage19d_{run['run_id']}_pair_distance_overlap.png"
    ...
    field_path = WORK_PLOT_DIR / f"stage19d_{run['run_id']}_field_snapshot.png"
    ...
    if env is not None:
        env_path = WORK_PLOT_DIR / f"stage19d_{run['run_id']}_envelope_snapshot.png"
    ...
    result = {
        ...
        'mixed_norm_series': mixed_norm_series,
...
def classify_base_rows(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    ...
    for row in rows:
        ...
        row['mixed_transport_stabilized'] = mixed_gain
        persistence_gain = (
        ...
        gate_hit = (
            persistence_gain
            and (mixed_gain == 1 or dwell_delta >= 0.05)
            and float(row['max_local_kernel_deformation']) <= 0.05 + 1.0e-12
        )
        row['gate_met'] = int(gate_hit)
        if gate_hit:
            row['output_label'] = 'transient mixed substrate response'
            score = (
...
def create_summary_plots(rows: list[dict[str, Any]], branch_kind: str) -> list[Path]:
    ...
    comparison_name = 'stage19d_mixed_scalar_vector_comparison.png'
    ...
    fig.suptitle('Stage 19D null vs mixed scalar-vector substrate reinforcement')
    fig.savefig(comparison, dpi=180, bbox_inches='tight')
    plt.close(fig)
    plot_paths.append(comparison)

    kernel_name = 'stage19d_mixed_scalar_vector_summary.png'
    ...
    for idx, cond in enumerate(conds):
        ...
        for rep in reps:
            ...
            supports.append(float(row['corridor_dwell_time']))
    ...
    axes[1].set_title('Corridor dwell time')
    for ax in axes:
        ax.set_xticks(x)
        ax.set_xticklabels(reps, rotation=20, ha='right')
        ax.grid(alpha=0.25, axis='y')
    ...
    response_name = 'stage19d_mixed_scalar_vector_response_matrix.png'
    response = WORK_PLOT_DIR / response_name
    label_map = {
        'no mixed substrate effect': 0,
        'transient mixed substrate response': 1,
        'scale-fragile mixed substrate-assisted persistence': 2,
        'scale-stable mixed substrate-assisted persistence': 3,
    }
    ...
    ax.set_title('Stage 19D mixed scalar-vector response matrix')
    plt.colorbar(im, ax=ax, fraction=0.045, pad=0.04)
...
def write_note(path: Path, json_rel: str, csv_rel: str, rows: list[dict[str, Any]], label_counts: dict[str, int], promoted: dict[str, Any] | None, branch_kind: str) -> None:
    title = '# Stage 19D Mixed Scalar + Vector Substrate Reinforcement v1'
    description = 'This Stage 19D pilot tests whether bounded mixed scalar-density and vector-flux substrate reinforcement can produce a persistence gain that neither channel produced alone.'
    lines = [
        title,
        '',
    ...
    for row in rows:
        lines.extend([
            ...
            f"  - scalar field norm: `{row['scalar_field_norm']:.4f}`",
            f"  - vector field norm: `{row['vector_field_norm']:.4f}`",
            f"  - mixed field norm: `{row['mixed_field_norm']:.4f}`",
            f"  - kernel deviation norm: `{row['kernel_deviation_norm']:.4f}`",
            f"  - scalar signal strength: `{row['scalar_signal_strength']:.4f}`",
            f"  - vector signal strength: `{row['vector_signal_strength']:.4f}`",
            f"  - scalar support fraction: `{row['scalar_support_fraction']:.4f}`",
            ...
            f"  - energy dispersion shift: `{row['energy_dispersion_shift']:.4f}`",
            f"  - mixed transport stabilized: `{row['mixed_transport_stabilized']}`",
...
def main() -> None:
    ...
    runsheet = normalize_stage19d_runsheet(load_runsheet(args.runsheet))
    branch_kind = str(runsheet.get('branch_kind', 'mixed_scalar_vector'))
    ...
    if not runs:
        raise SystemExit('no Stage 19D runs selected')
    ...
    result = {
        'stage': runsheet.get('stage', 'Stage 19D'),
        'description': runsheet['description'],
        ...
        'conclusion': (
            'Stage 19D tests whether minimal bounded mixed scalar-density and vector-flux substrate reinforcement can create persistence consequences on the projected transverse architecture.'
        ),
    }
