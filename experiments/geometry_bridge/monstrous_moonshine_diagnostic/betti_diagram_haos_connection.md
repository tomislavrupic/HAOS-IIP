...
The current formal scaffold is generated by:
...
The scaffold status is `OPEN`, because this repository does not currently
contain a local Lean project, local Lean graph definitions, or checked proofs.
...
The current Lean target ladder is:

```text
L1 finite graph component count exists
L2 graph isomorphism preserves component count
L3 D4 action induces graph isomorphism under the threshold relation
L4 D4 preserves Betti_0
L5 graph isomorphism preserves Betti_1
L6 D4 preserves [Betti_0, Betti_1]
```

All six are target-only in this repository as of the current scaffold.
