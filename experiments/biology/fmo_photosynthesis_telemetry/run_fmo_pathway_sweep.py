...
SPECS = [
    PathwaySpec("flux_light", 0.40, 0.18, 0.18),
    PathwaySpec("flux_medium", 0.38, 0.20, 0.34),
    PathwaySpec("flux_strong", 0.34, 0.24, 0.52),
    PathwaySpec("flux_modes3", 0.38, 0.20, 0.34, spectral_modes=3),
    PathwaySpec("flux_modes5", 0.38, 0.20, 0.34, spectral_modes=5),
    PathwaySpec("flux_assisted", 0.36, 0.20, 0.34, environment_assist_gain=0.08),
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 56.3 FMO pathway/flux modeling sweep.")
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs" / "robustness_56_3_pathway_flux")
...
def main() -> None:
    ...
    write_csv(args.output_dir / "fmo_pathway_flux_runs.csv", rows)
    write_json(args.output_dir / "fmo_pathway_flux_summary.json", summary)
    write_report(args.output_dir / "fmo_pathway_flux_report.md", summary)
    write_plot(args.output_dir / "fmo_pathway_flux_summary.png", summary)
...
def write_report(path: Path, summary: list[dict[str, Any]]) -> None:
    lines = [
        "# Phase 56.3 FMO Pathway/Flux Modeling Sweep",
        "",
        "This sweep adds explicit pathway-flux restoration to the FMO telemetry sidecar.",
        "It is a diagnostic refinement of Phase 56, not a biological validation claim.",
        "",
        "| spec | runs | pass_rate | recoverability | site_identity | pathway_identity | active_null_z | max_controls |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for item in sorted(summary, key=lambda row: row["pathway_identity_retention_mean"], reverse=True):
        lines.append(
            "| {spec} | {runs} | {pass_rate:.3f} | {rec:.6f} +/- {rec_std:.6f} | {site:.6f} +/- {site_std:.6f} | {path:.6f} +/- {path_std:.6f} | {z:.6f} +/- {z_std:.6f} | {controls} |".format(
                spec=item["spec"],
                runs=item["runs"],
...
def write_plot(path: Path, summary: list[dict[str, Any]]) -> None:
    ...
    axes[1].axhline(0.50, color="tab:red", linestyle="--", linewidth=1)
    ...
    fig.suptitle("Phase 56.3 FMO pathway/flux sweep")
    fig.tight_layout()
