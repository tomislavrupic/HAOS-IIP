...
GW_SPECIFICITY_STATUS = REPO_ROOT / "experiments/physics_bridge/gw_memory_entry_gate/event_window_specificity_status.json"
...
def build_rows() -> list[ClaimGateRow]:
    ...
    gw_specificity = load_status(GW_SPECIFICITY_STATUS)
    return [
        ClaimGateRow(
        ...
        ClaimGateRow(
            claim_id="gw_event_window_specificity_proxy",
            claim_or_probe="GW event-window specificity proxy",
            status=str(gw_specificity["bridge_status"]),
            scope=(
                "Event-internal strain-derived specificity using ridge-coherence and "
                "analytic-phase continuity metrics against envelope-preserving controls."
            ),
            supporting_artifact=artifact(GW_SPECIFICITY_STATUS),
            promotion_gate=(
                "Requires mean specificity margin >= 0.25, min margin >= 0.12, "
                "and zero false-positive replicate leakage before bounded PASS."
            ),
            allowed_language="event-internal strain-proxy specificity partially improved",
            disallowed_language="real gravitational memory detected; event-window specificity closed; GW memory observed",
            notes=(
                f"mean_margin={gw_specificity['mean_specificity_margin']:.6f}; "
                f"min_margin={gw_specificity['min_specificity_margin']:.6f}; "
                f"highest_control={gw_specificity['highest_control_sample']}"
            ),
        ),
        ClaimGateRow(
        ...
        ClaimGateRow(
            ...
            notes="Phases 61, 64, 65, and 66 are proxy gates only; they do not close this row.",
        ),
        ClaimGateRow(
...
The Phase 66 event-internal specificity row is also `MARGINAL`: ridge and phase metrics improve separation, but partial-overlap, micro-window, and sliding-window controls still prevent a bounded PASS.
...
Phase 67 should either replicate the Phase 64-66 entry gate on curated local GWOSC strain files or run an event-window metric ablation to identify which specificity term is actually carrying the separation. In both cases, real gravitational-memory observable and BMS/soft-theorem rows remain `OPEN` until directly tested.
