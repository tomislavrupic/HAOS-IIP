...
def build_rows() -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    rows.append(
        row(
            ...
            "All tested disorder seeds now pass the same native-flux thresholds after extending the delayed asymptotic fit window to [0.006, 0.024]."
            if min_seed_pass == max_seed_count
            else "Aggregates pass, but one tested case still has a marginal seed-level failure.",
        )
    )

    metadata = {
        "program": "HAOS-IIP physics bridge 53.0",
        "bridge_state": "updated through frozen follow-on releases 53.2, 53.3, and 53.4",
        "claim_type": "post-processing proxy dictionary over frozen scalar artifacts",
        "synchronization_note": (
            "This generated bridge summary reflects the frozen 53.4 state: "
            "weak localized bump response passes on delayed fit window [0.010, 0.026], "
            "shell-native power-law scaling passes while raw local-gradient scaling remains open, "
            "and the seed-universal disorder margin now passes at 3/3 after extending the delayed "
            "asymptotic fit window to [0.006, 0.024]."
        ),
        "non_claims": [
